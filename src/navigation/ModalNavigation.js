...
const buttonCloseLight = () => {
	return (
		<IconButton icon={"window-close"} color={"#fff"} style={{marginLeft:15}} size={24} onPress={() => props.navigation.goBack()}/>
		)
};

const buttonCloseColor = () => {
	return (
		<IconButton icon={"window-close"} color={ColorsApp.PRIMARY} style={{marginLeft:15}} size={24} onPress={() => props.navigation.goBack()}/>
		)
};
